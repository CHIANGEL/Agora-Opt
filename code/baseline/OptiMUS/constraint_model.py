...
def extract_formulation_from_end(text):
    ...
    if "NEW VARIABLES" not in json_res:
        json_res["NEW VARIABLES"] = {}
    if "FORMULATION" not in json_res:
        json_res["FORMULATION"] = formulation
    if "AUXILIARY CONSTRAINTS" not in json_res:
        json_res["AUXILIARY CONSTRAINTS"] = auxiliaries
        ...
        for c in formulated_constraints.copy():
            for q in qs[0:1]:
                ...
                while k > 0:
                    ...
                    x = get_response(p, model=model,temperature=temperature)
