...
Where SYMBOL is a string representing the parameter (use CamelCase), SHAPE is the shape of the parameter (e.g. "[]" for scalar, or "[N, M]" for a matrix of size N x M where N and M are scalar parameters), DEFINITION is a string describing the parameter, and type is one of "int", "float", or "binary". VALUE is the value of the parameter. For example:
...
def get_params(desc, model, temperature, check=True):
    print("Extracting parameters...")
    k = 5
    while k > 0:
        try:
            res = get_response(prompt_params.format(description=desc), model, temperature=temperature)
        ...
        for q, func in qs:
            for param in params.copy():
                ...
                while k > 0:
                    ...
                    x = get_response(prompt, model=model)
