...
def execute_code(dir, code_filename):
    try:
        ...
        with open(os.path.join(dir, f"code_output.txt"), "w") as f:
...
def execute_and_debug(state, dir, model, temperature, logger, max_tries=3):
    ...
    for iteration in range(max_tries):
        ...
        if status == "Success":
            ...
            response = get_response(p, model=model, temperature=temperature)
