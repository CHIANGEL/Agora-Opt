...
'''
    ...
    def forward(self, problem, comment_pool):
        ...
        output = safe_json_loads(output, default=[])
        ...
        answer = ''
        for item in output:
            terminology = item.get('terminology', '') if isinstance(item, dict) else ''
            interpretation = item.get('interpretation', '') if isinstance(item, dict) else ''
            if terminology or interpretation:
                answer += f'{terminology}:{interpretation}\n'
